...
class JsonVisualizer {
  ...
  createHeaderBar() {
    ...
    title.textContent = t('viz.outputTitle');
    ...
    pathLabel.textContent = t('viz.pathFormat');
    ...
    const formats = [
      { value: 'jsonpath', text: t('viz.pathFormat.jsonpath') },
      { value: 'python-dict', text: t('viz.pathFormat.pythonDict') },
      { value: 'python-data', text: t('viz.pathFormat.pythonData') },
      { value: 'javascript', text: t('viz.pathFormat.javascript') },
      { value: 'javascript-data', text: t('viz.pathFormat.javascriptData') },
      { value: 'dot-notation', text: t('viz.pathFormat.dotNotation') }
    ];
    ...
    searchInput.placeholder = t('viz.searchPlaceholder');
    searchInput.disabled = true;
    
    const searchBtn = document.createElement('button');
    searchBtn.className = 'json-search-btn';
    searchBtn.innerHTML = '🔍';
    searchBtn.title = t('viz.searchBtn');
    ...
    fullscreenBtn.title = t('viz.fullscreen');
  ...
  toggleFullscreen() {
    ...
    if (btnGroup && !btnGroup.querySelector('.json-fs-exit-btn')) {
      ...
      exitBtn.innerHTML = '<svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3"/></svg> ' + t('viz.exitFullscreen');
  ...
  disableSearchFeatures() {
    ...
    if (searchInput && searchBtn) {
      searchInput.disabled = true;
      searchBtn.disabled = true;
      searchInput.placeholder = t('viz.searchPlaceholderDisabled');
      searchInput.value = '';
    }
  ...
  createCopyPathButton(path) {
    ...
    copyBtn.innerHTML = '◈';
    copyBtn.title = `${t('viz.copyPathTitle')} ${this.convertPath(path, this.pathFormat)}`;
  ...
  createCopyContentButton(path, data) {
    ...
    copyBtn.innerHTML = '📋';
    copyBtn.title = t('viz.copyContent');
    copyBtn.dataset.path = path;
  ...
  updateAllCopyButtons() {
    ...
    copyButtons.forEach(button => {
      ...
      if (originalPath) {
        ...
        button.title = `${t('viz.copyPathTitle')} ${convertedPath}`;
      }
    });
  }
}
